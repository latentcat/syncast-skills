...
description: 通过浏览器内 Agent Action Layer 操作已打开的 Syncast 项目。当用户要在 Syncast 项目中工作时，必须先连接项目，并通过项目 Action 完成生成、文档、资源、频道、时间轴和 Agent 操作；若用户未说明是项目内工作还是仅在项目外查看独立资产，必须先询问，禁止默认走直接生成 CLI。
---
